...
class GaussianMixture(Distribution):
    ...
    def rsample(self):
        z = (
                ...
                MultivariateDiagonalNormal(
                    torch.zeros(self.normal_means.size()),
                    torch.ones(self.normal_stds.size())
                ).sample()
        )
        z.requires_grad_()
        c = self.categorical.sample()[:, :, None]
    ...
    def mle_estimate(self):
        ...
        """
        c = torch.zeros(self.weights.shape[:2])
...
class TanhNormal(Distribution):
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        correction = - 2. * (
            torch.from_numpy(np.log([2.])).to(self.device)
            - pre_tanh_value
    ...
    def log_prob(self, value, pre_tanh_value=None):
        if pre_tanh_value is None:
            ...
            value = torch.clamp(value, -0.999, 0.999).to(self.device)
